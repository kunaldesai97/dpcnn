...
@bp.route('/write_pred', methods=['POST'])
def write_pred():
    headers = request.headers
    # check header here
    content = request.get_json()
...
@bp.route('/write_input', methods=['POST'])
def write_input():
    headers = request.headers
    # check header here
    content = request.get_json()
...
def scan():
    ...
    table_name = objtype.capitalize()
    table = dynamodb.Table(table_name)
    response = table.scan(Select='ALL_ATTRIBUTES', FilterExpression = Attr('label').eq(objkey))
    return response
